...
{
    Pessoa regPessoa;
    char nomeBuffer[126]; // possível overflow
...
void Form::stressTest(int quantidade)
{
    auto task = []() {
        Pessoa regPessoa = {
            "Teste Nome",
            "teste" + to_string(rand()) + "@email.com",
            to_string(rand() % 10000000000 + 10000000000),
            "11" + to_string(rand() % 1000000000 + 100000000),
            "01/01/1990",
            "SenhaF0rte",
            dataAtual()
        };
...
int main()
{
    ...
    do {
        ...
        else if (opcao == 2) app.stressTest(10000);
    } while (opcao != 3);
